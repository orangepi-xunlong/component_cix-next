...
static struct devfreq_simple_ondemand_data gpu_devfreq_data;
...
static int kbase_devfreq_target(struct device *dev, unsigned long *target_freq, u32 flags)
{
...
#else
	/* DPTSW-13014: Applying the core mask downscaling before adjusting the
	 * frequency can reduce the runtime in the core mask taking effect.
	 */
	...
	for (j = 0; j < PM_CLK_MAX_RETRY_TIMES; j++) {
		current_gpu_level = ioread32(kbdev->clk_response_addr);

		if (current_gpu_level == nominal_freq_mhz)
			break;
		udelay(2);
	}

	if (current_gpu_level != nominal_freq_mhz) {
		dev_dbg(dev, "set clock (%lu) is inconsistent with clock read (%lu) from PM\n",
					nominal_freq_mhz, current_gpu_level);
	}
...
/**
 * Use sky1_gpu_init_perf_opp_table() to init perf opp table
 * from scmi interface for sky1 platform.
 */
#if !IS_ENABLED(CONFIG_ARM_SCMI_PERF_DOMAIN)
...
static void kbase_devfreq_term_freq_table(struct kbase_device *kbdev)
{
	...
	/* Remove the opp tables created from pm firmware */
#if IS_ENABLED(CONFIG_ARM_SCMI_PERF_DOMAIN)
	sky1_remove_opp_table(kbdev, dp);
#endif
	kfree(dp->freq_table);
	dp->freq_table = NULL;
}
...
int kbase_devfreq_init(struct kbase_device *kbdev)
{
	...
	kbdev->current_nominal_freq = kbdev->current_freqs[KBASE_CLOCK_DOMAIN_SHADER_CORES];

	dp = &kbdev->devfreq_profile;

	dp->initial_freq = kbdev->current_freqs[KBASE_CLOCK_DOMAIN_SHADER_CORES];
	dp->polling_ms = 100;
...
#else
	if (sky1_gpu_init_perf_opp_table(kbdev, dp))
		return -EFAULT;
#endif
	...
	gpu_devfreq_data.upthreshold = 60;
	gpu_devfreq_data.downdifferential = 20;
	kbdev->devfreq = devfreq_add_device(kbdev->dev, dp, "simple_ondemand", &gpu_devfreq_data);
...
#if IS_ENABLED(CONFIG_ARM_SCMI_PERF_DOMAIN)
	kbdev->sky1_nb_call.notifier_call = sky1_gpu_clk_rate_change_notifier;
	devm_devfreq_register_notifier(kbdev->dev, kbdev->devfreq,
									&kbdev->sky1_nb_call, DEVFREQ_TRANSITION_NOTIFIER);
#endif
...
#if IS_ENABLED(CONFIG_ARM_SCMI_PERF_DOMAIN)
	devm_devfreq_unregister_notifier(kbdev->dev, kbdev->devfreq,
									&kbdev->sky1_nb_call, DEVFREQ_TRANSITION_NOTIFIER);
#endif
