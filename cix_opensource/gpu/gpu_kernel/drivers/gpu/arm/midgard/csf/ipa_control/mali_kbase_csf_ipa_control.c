...
static u64 read_value_cnt(struct kbase_device *kbdev, u8 type, u8 select_idx)
{
	...
	case KBASE_IPA_CORE_TYPE_CSHW:
		return kbase_reg_read64_coherent(kbdev, IPA_VALUE_CSHW_OFFSET(select_idx));

	case KBASE_IPA_CORE_TYPE_MEMSYS:
		return kbase_reg_read64_coherent(kbdev, IPA_VALUE_MEMSYS_OFFSET(select_idx));

	case KBASE_IPA_CORE_TYPE_TILER:
		return kbase_reg_read64_coherent(kbdev, IPA_VALUE_TILER_OFFSET(select_idx));

	case KBASE_IPA_CORE_TYPE_SHADER:
		return kbase_reg_read64_coherent(kbdev, IPA_VALUE_SHADER_OFFSET(select_idx));
	case KBASE_IPA_CORE_TYPE_NEURAL:
		if (kbase_csf_dev_has_ne(kbdev))
			return kbase_reg_read64_coherent(kbdev, IPA_VALUE_NEURAL_OFFSET(select_idx));
		else
			return 0;
	default:
