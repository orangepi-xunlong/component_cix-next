...
static int kbase_common_reg_map(struct kbase_device *kbdev)
{
	...
	if (!request_mem_region(kbdev->rcsu_reg_start, kbdev->rcsu_reg_size, dev_name(kbdev->dev))) {
		dev_err(kbdev->dev, "RCSU register window unavailable\n");
		err = -EIO;
		goto out_region;
	}

	kbdev->rcsu_reg = ioremap(kbdev->rcsu_reg_start, kbdev->rcsu_reg_size);
	if (!kbdev->rcsu_reg) {
		dev_err(kbdev->dev, "Can't remap rcsu register window\n");
		err = -EINVAL;
		goto out_rcsu_ioremap;
	}
#endif
	return err;
#if IS_ENABLED(CONFIG_ARCH_CIX)
out_rcsu_ioremap:
	release_mem_region(kbdev->rcsu_reg_start, kbdev->rcsu_reg_size);
#endif
out_ioremap:
...
#if IS_ENABLED(CONFIG_ARCH_CIX)
	if (kbdev->rcsu_reg) {
		iounmap(kbdev->rcsu_reg);
		release_mem_region(kbdev->rcsu_reg_start, kbdev->rcsu_reg_size);
		kbdev->rcsu_reg = NULL;
		kbdev->rcsu_reg_start = 0;
		kbdev->rcsu_reg_size = 0;
	}
#endif
}
...
int registers_map(struct kbase_device *const kbdev)
{
	...
	struct resource *rcsu_reg_res;
	int err;

#if IS_ENABLED(CONFIG_ARCH_CIX)
	rcsu_reg_res  = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	reg_res = platform_get_resource(pdev, IORESOURCE_MEM, 1);
#else /* !IS_ENABLED(CONFIG_ARCH_CIX) */
...
#if IS_ENABLED(CONFIG_ARCH_CIX)
	if (!rcsu_reg_res) {
		dev_err(kbdev->dev, "Invalid rcsu register resource\n");
		return -ENOENT;
	}
	kbdev->rcsu_reg_start = rcsu_reg_res->start;
	kbdev->rcsu_reg_size = resource_size(rcsu_reg_res);
#endif
...
int power_control_init(struct kbase_device *kbdev)
{
	...
	int err = 0;
	unsigned int i, k;
	static const char *clk_names[BASE_MAX_NR_CLOCKS_REGULATORS] = {"gpu_clk_core", "gpu_clk_stacks"};
	static const char *backup_clk_names[MAX_GPU_BACKUP_CLOCKS] = { "gpu_clk_200M", "gpu_clk_400M" };

	if (has_acpi_companion(kbdev->dev))
		dev_pm_set_driver_flags(kbdev->dev, DPM_FLAG_NO_DIRECT_COMPLETE);

#if defined(CONFIG_REGULATOR)
		/* Regulator array must be terminated with a NULL pointer, because of how the
		 * Linux kernel iterates through them.
		 */
		static const char *const regulator_names[] = { "mali", "shadercores", NULL };
		 ...
		 */
		for (i = 0; i < BASE_MAX_NR_CLOCKS_REGULATORS; i++) {
			kbdev->clocks[i] = devm_clk_get(&pdev->dev, clk_names[i]);
			...
			if (err) {
				...
				devm_clk_put(kbdev->dev, kbdev->clocks[i]);
				break;
			}
		}
		...
		for (i = 0; i < MAX_GPU_BACKUP_CLOCKS; i++) {
			kbdev->backup_clocks[i] = devm_clk_get(&pdev->dev, backup_clk_names[i]);
			if (IS_ERR(kbdev->backup_clocks[i])) {
				err = PTR_ERR(kbdev->backup_clocks[i]);
				kbdev->backup_clocks[i] = NULL;
				break;
			}

			err = clk_prepare_enable(kbdev->backup_clocks[i]);
			if (err) {
				dev_err(kbdev->dev, "Failed to prepare and enable backup clock (%d)\n", err);
				devm_clk_put(kbdev->dev, kbdev->backup_clocks[i]);
				break;
			}
		}
		if (err == -EPROBE_DEFER) {
		...
		kbdev->bk_clocks = i;
		dev_dbg(&pdev->dev, "Backup clocks probed: %u\n", kbdev->bk_clocks);
...
regulators_probe_defer:
		...
		for (i = 0; i < MAX_GPU_BACKUP_CLOCKS; i++) {
			if (kbdev->backup_clocks[i]) {
				if (__clk_is_enabled(kbdev->backup_clocks[i]))
					clk_disable_unprepare(kbdev->backup_clocks[i]);
				devm_clk_put(kbdev->dev, kbdev->backup_clocks[i]);
				kbdev->backup_clocks[i] = NULL;
			} else
...
/* Global flag used to determine if sky1 power model timer is enabled */
extern bool enable_sky1_power_model;

/* Get the value of sky1 power model timer switch*/
static ssize_t enable_sky1_power_model_show(struct device *dev, struct device_attribute *att, char *buf)
{
    return sprintf(buf, "%d\n", enable_sky1_power_model);
}

/* Set the value of sky1 power model timer switch*/
static ssize_t enable_sky1_power_model_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct kbase_device *kbdev = dev_get_drvdata(dev);
	unsigned int val;
	...
	} else if (val == 0 && enable_sky1_power_model) {
		enable_sky1_power_model = false;
		hrtimer_cancel(&kbdev->sky1_power_timer);
		/* Clear the power data in the shared address */
...
static DEVICE_ATTR_RW(enable_sky1_power_model);
...
static struct attribute *kbase_attrs[] = {
	...
	&dev_attr_enable_sky1_power_model.attr,
#endif /* !MALI_USE_CSF */
