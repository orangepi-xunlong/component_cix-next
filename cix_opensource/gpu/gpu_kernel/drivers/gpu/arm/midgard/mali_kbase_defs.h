...
/* Maximum number of backup clock pairs that needs to be temporarily
 * switched during the GPU DFS process.
 */
#define MAX_GPU_BACKUP_CLOCKS 2
...
struct kbase_device {
	...
	struct device *dev;
	struct device *sky1_power_dev;
	struct device *sky1_perf_dev;
	struct reset_control *gpu_reset;
#if IS_ENABLED(CONFIG_ARM_SCMI_PERF_DOMAIN)
	struct notifier_block sky1_nb_call;
#endif
	struct miscdevice mdev;
	u64 reg_start;
	size_t reg_size;
	void __iomem *reg;
	void __iomem *clk_response_addr;
	void __iomem *dynamic_power_addr;
	struct hrtimer sky1_power_timer;
	struct workqueue_struct *sky1_power_wq;
	struct work_struct sky1_power_work;
#if IS_ENABLED(CONFIG_ARCH_CIX)
	u64 rcsu_reg_start;
	size_t rcsu_reg_size;
	void __iomem *rcsu_reg;
	u64 harvesting_core_mask;
#endif
	struct {
		void __iomem **regs;
		u32 *flags;
	...
	struct clk *backup_clocks[MAX_GPU_BACKUP_CLOCKS];
	unsigned int nr_clocks;
	unsigned int bk_clocks;
