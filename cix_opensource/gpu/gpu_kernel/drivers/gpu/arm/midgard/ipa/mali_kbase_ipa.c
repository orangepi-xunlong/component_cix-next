				  ...
				  size_t num_elems, bool dt_required)
{
	...
	if (has_acpi_companion(model->kbdev->dev)) {
		struct fwnode_handle *fwnode = device_get_named_child_node(model->kbdev->dev, "power_model");
		const char **s_vals = kzalloc(sizeof(*s_vals) * num_elems, GFP_KERNEL);
		if (!s_vals) {
			err = -ENOMEM;
		} else {
			int r_cnt = fwnode_property_read_string_array(fwnode, name, s_vals, num_elems);
			if (r_cnt == num_elems)
				for (int i = 0; i < r_cnt; i++) {
					err = kstrtoint(s_vals[i], 10, addr + i);
					if (err)
						break;
				}
				     ...
				     size_t size, bool dt_required)
{
	...
	if (has_acpi_companion(model->kbdev->dev)) {
		struct fwnode_handle *fwnode = device_get_named_child_node(model->kbdev->dev, "power_model");
		err = fwnode_property_read_string(fwnode, name, &string_prop_value);
	} else {
		struct device_node *model_dt_node = get_model_dt_node(model, dt_required);
...
static u32 kbase_scale_dynamic_power(const u32 c, const u32 freq, const u32 voltage)
{
	...
	const u32 v3 = (voltage * voltage * voltage) / 1000000;

	const u32 mv = voltage;

	const u32 f_MHz = freq / 1000000;

	const u32 f2 = (f_MHz * f_MHz) / 1000;

	const u32 f3 = (f_MHz * f_MHz * f_MHz) / 1000000;

	const u64 v_coeff = (4664895 * v2 - 2817303 * v3 - 1201941 * mv) / 1000000;

	const u64 f_coeff = (1978725 * f3 + 3666507 * f_MHz - 4628697 * f2) / 1000000;

	const u64 p_coeff =  v_coeff * f_coeff;

	return div_u64(p_coeff * ((c / 1000)), 1000000);
#endif
}
...
#endif /* !CONFIG_MALI_CIX_POWER_MODEL */
...
u32 get_sky1_static_power(struct kbase_device *kbdev)
{
	/* The GPU static power is calculated in the PM_FW and updated to
	 * the address 0x83BF0644.
	 */
	return mali_readl(kbdev->dynamic_power_addr + 4);;
}
#if !IS_ENABLED(CONFIG_MALI_CIX_POWER_MODEL)
...
#else
static u32 sliding_power_average(u32 power) {
	static u32 buffer[50] = {0};
	static int index = 0;
	...
	/* The sliding cycle is calculated by the IPA sampling period */
	index = (index + 1) % 50;
	if (!is_buffer_full && index == 0) {
		is_buffer_full = 1;
	}

	return sum / (is_buffer_full ? 50 : index);
}
	...
	unsigned long voltage)
{
	...
	/* Dynamic power and leakage power should be 0 when GPU is powered off */
	...
	if (err && model != kbdev->ipa.fallback_model) {
		dev_dbg(kbdev->dev, "get dynamic coeff from sky1 power model failed : %d\n", err);
		model = kbdev->ipa.fallback_model;
		err = model->ops->get_dynamic_coeff(model, power_coeffs);
	}
	...
	/* For the sky1 power model, the counters related to the GPU_TOP and
	 * GPU_CORE level need to be combined for calculation. If they are
	 * calculated separately, there may be a situation where the coefficient
	 * is a negative number. Here, all the counters are combined and
	 * calculated at the top level.
	 */
	*power = kbase_scale_dynamic_power(power_coeffs[KBASE_IPA_BLOCK_TYPE_TOP_LEVEL],
			freqs[KBASE_IPA_BLOCK_TYPE_TOP_LEVEL],
			volts[KBASE_IPA_BLOCK_TYPE_TOP_LEVEL]);

	*power += get_sky1_static_power(kbdev);

	*power = sliding_power_average(*power);
...
void kbase_ipa_reset_data(struct kbase_device *kbdev)
{
	...
	if (elapsed_time > RESET_INTERVAL_MS) {
		...
		/* The counters related to sky1 power model do not
