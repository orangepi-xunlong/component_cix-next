...
//Frist frame, all the vout used the reserved buffer
int armcb_init_output_addr_by_i7 (uint32_t *p_vout_reg1,
                                     uint32_t *p_vout_reg2, int vout)
{
	int rc = 0;
	uint32_t vout_reg1 = 0;
	uint32_t vout_reg2 = 0;
...
#ifdef V4L2_OPT
	pstream->reserved_buf_addr_dma = discard_buf_addr_dma;

	if (!pbuf) {
		LOG(LOG_DEBUG, "[Stream#%d] no empty buffers",
			pstream->stream_id);
		startaddr = pstream->reserved_buf_addr_dma;
...
static void armcb_isp_irq_sol_i7(unsigned int ctx_id)
{
	...
	for (i = 0; i < V4L2_STREAM_TYPE_MAX; i++) {
		...
		if (atomic_read(&pdev->upload_streamoff) == 0) {
...
#ifdef V4L2_OPT
	ret = armcb_v4l2_release_discard_buffer(RESERVED_BUF_SIZE, &pdev->dev);
	if (ret < 0)
		LOG(LOG_ERR, "Alloc reserved buffer failed ! \n");
#endif
