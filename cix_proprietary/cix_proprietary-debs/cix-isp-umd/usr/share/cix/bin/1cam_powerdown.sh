...
# GPIO num
gpio_base_num=64
gpio_pwen_num=18+$gpio_base_num
gpio_pwdn_num=15+$gpio_base_num
gpio_rst_num=12+$gpio_base_num


# export GPIO
echo $gpio_pwen_num > /sys/class/gpio/export
echo $gpio_pwdn_num > /sys/class/gpio/export
...
cd /sys/class/gpio/gpio$gpio_pwdn_num || exit
echo "out" > direction
echo 0 > value
...
echo $gpio_pwdn_num > /sys/class/gpio/unexport
