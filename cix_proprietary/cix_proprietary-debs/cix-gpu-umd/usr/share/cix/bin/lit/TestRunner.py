...
def _runShTest(test, litConfig, useExternalSh, script, tmpBase):
    ...
    cmd = '\n'.join(script)
    env = test.config.environment
    # replace env variables recursively until no more substitutions can be done
    while True:
        cmd_update = cmd
        replace = lambda match: env.get(match.group(1), match.group(0))
        cmd_update = re.sub("\$\{?\(?([^\d\W]\w*)\)?\}?", replace, cmd, re.MULTILINE)
        # if nothing was replaced, exit
        if cmd == cmd_update:
            break
        # otherwise update cmd
        cmd = cmd_update
